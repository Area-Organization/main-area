...
<div
  class="h-full flex flex-col gap-2 bg-sidebar text-sidebar-foreground rounded-xl border border-sidebar-border p-2 overflow-hidden"
>
  ...
  <div class="flex flex-col gap-1 overflow-y-auto custom-scrollbar flex-1 pr-1">
    {#each services as service}
      ...
      <div
        class={cn(
          "rounded-lg border transition-all duration-200 shrink-0",
          isOpen ? "bg-background border-border shadow-sm" : "bg-transparent border-transparent hover:bg-muted/50"
        )}
      >
        <button
          onclick={() => connected && toggle(service.name)}
          class={cn(
            "w-full flex items-center gap-2 p-1.5 text-left bg-transparent rounded-lg transition-colors",
        ...
        >
          ...
          {#if !connected}
            <div
              class="flex items-center gap-1 text-[10px] text-muted-foreground bg-muted/50 px-1.5 py-0.5 rounded-full"
            >
          ...
          <div class="px-2 pb-2 pl-7 space-y-1" transition:slide={{ duration: 200, axis: "y" }}>
            ...
            {#if items && items.length > 0}
              {#each items as item}
                ...
                >
                  <GripVertical
                    class="h-3 w-3 mt-0.5 text-muted-foreground/40 group-hover:text-muted-foreground shrink-0"
                  />
